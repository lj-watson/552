...
enum RPTState {
  INIT,       // Initial or this entry experienced stride
              // that is different from the steady state
  STEADY,     // Prediction should be stable for a while
  TRANSIENT,  // System unsure if current stride is stable
  NO_PRED,    // Disables prefetching for entry
};

typedef struct {
  md_addr_t tag;              // Tag corresponding to address of memory instruction (PC)
  md_addr_t prev_addr;        // Last address referenced by corresponding instruction
  long long stride;           // The difference between the last 
                        // 2 addresses generated by PC (can be negative)
  enum RPTState state;  // 2-bit encoding (4 states) of the past history
} RPTEntry;
...
void stride_prefetcher(struct cache_t *cp, md_addr_t addr) {
  /* Initialize RPT if does not exist yet: */
  if (RPT == NULL) {
    rpt_size = cp->prefetch_type;
    RPT = (RPTEntry*)malloc(rpt_size*sizeof(RPTEntry));
  ...
  md_addr_t PC = get_PC();
  unsigned int index = (PC >> 3) % rpt_size; // The last 3 bits of PC are always 0
  RPTEntry* entry = &RPT[index];

  if (PC == entry->tag) {
    long long stride = (long long)addr - (long long)entry->prev_addr;
    ...
    /* RPT entry doesn't exist yet, update tag and prev_addr: */
    entry->tag = PC;
    entry->prev_addr = addr;
    entry->stride = 0;
    entry->state = INIT;
