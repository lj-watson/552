...
typedef struct {
  ...
  int prev_addr;        // Last address referenced by corresponding instruction
  int stride;           // The difference between the last 
...
void stride_prefetcher(struct cache_t *cp, md_addr_t addr) {
  ...
  int index = (PC >> 3) & (rpt_size - 1); // The last 3 bits of PC are always 0
